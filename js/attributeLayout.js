...
(function (exports) {
  ...
  const computeAttributePositions = (graph, newAttrNodes) => {
    const estimateAttributeHalfSize =
      window.ERBuilder.estimateAttributeHalfSize;
    ...
    const relAnglesByEntity = new Map();
    graph.getEdges().forEach((e) => {
      const em = e.getModel();
      if (
    ...
    entityOrder.forEach((entityId) => {
      ...
      attrs.forEach((attr, i) => {
        ...
        const attrBorderTowardEnt = (px, py) => {
          const dx2 = ent.x - px;
          const dy2 = ent.y - py;
  ...
  const showAttributes = ({
    ...
    const { buildAttributeData, patchRelationshipLinkPoints } = window.ERBuilder;
