...
export type BuildExportSVGCallback = (
    err: unknown,
    result?: BuildExportSVGResult | null,
) => void;
...
export function buildExportSVG(
    ...
    try {
        ...
        tempGraph.read(data);
        patchRelationshipLinkPoints(tempGraph as unknown as GraphLike);

        setTimeout(() => {
            try {
                const svgElement = tempGraph.get("canvas").get("el") as SVGSVGElement;
...
export function exportSVG(options: ExportSVGOptions): void {
    ...
    const finishErr = (msg: string) => {
        ...
        if (onDone) onDone(new Error(msg), null);
    };
    ...
    buildExportSVG(options, (err, result) => {
        if (err || !result) {
            ...
            finishErr("SVG生成失败: " + errorMessage(err));
            return;
        }
...
function rasterizeSVGToPNG(
    ...
    img.onload = () => {
        try {
            ...
            if (!ctx) {
                URL.revokeObjectURL(url);
                cb(new Error("canvas getContext 返回空"));
                return;
            }
...
export function exportPNG(options: ExportPNGOptions): void {
    ...
    buildExportSVG(options, (err, result) => {
        if (err || !result) {
            ...
            finishErr("导出PNG失败: " + errorMessage(err));
            return;
        }
        rasterizeSVGToPNG(
            result.svgString,
            ...
            (rErr, blob) => {
                if (rErr || !blob) {
                    ...
                    finishErr("导出PNG失败: " + errorMessage(rErr));
                    return;
                }
...
function buildVertexStyle(model: DrawioStyleSource): string {
    ...
    const dashed =
        Array.isArray(s.lineDash) && s.lineDash.length ? "dashed=1;" : "";
    const labelFontColor =
        (model.labelCfg && model.labelCfg.style && model.labelCfg.style.fill) ||
        "#1e293b";
...
export function exportDrawio(options: ExportDrawioOptions): void {
    const { graphRef, hasGraph, onError, onDone, patchRelationshipLinkPoints } =
        options;
    ...
    try {
        ...
        finishErr("导出Drawio失败: " + errorMessage(err));
    }
}
