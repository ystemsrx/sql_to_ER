      ...
      const tryAngleWithFlags = (angle: number, flags: any, maxROverride?: number) => {
        ...
        for (let R = minR; R <= maxR; R += STEP) {
          ...
          if (flags.rectPierce) {
            for (let k = 0; k < obstacleEdges.length; k++) {
              const e = obstacleEdges[k];
              if (segmentHitsRect(e.x1, e.y1, e.x2, e.y2, px, py, attrHW, attrHH)) {
                bad = true;
                break;
              }
            }
...
export const hideAttributes = (graph: MutableGraph | null | undefined) => {
  ...
  const attrNodes = graph
    ...
    .slice();
  const attrIds = new Set(attrNodes.map((n) => (n.getModel() as ERNodeModel).id));
...
export interface ShowAttributesOptions {
  ...
  isColored: boolean;
  fontScale?: number;
  updateStyles?: (graph: GraphLike, isColored: boolean, fontScale?: number) => void;
}
...
export const showAttributes = ({
  ...
  graph.setAutoPaint(false);
  validAttrs.forEach((n) => graph.addItem("node", n as unknown as Record<string, unknown>));
  validEdges.forEach((e) => graph.addItem("edge", e as unknown as Record<string, unknown>));
  graph.paint();
  graph.setAutoPaint(true);

  if (typeof updateStyles === "function") {
    updateStyles(graph, isColored, safeFontScale);
  }
