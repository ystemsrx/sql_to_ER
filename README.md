...
5. 若图形较复杂，只需将每个矩形（实体）拖到大致位置，再点击 **「智能优化」**，即可自动整理布局
...
> 4. **实体 / 关系特别多时**，可先点击画布左上角的 **「隐藏属性」**，先把矩形（实体）和菱形（关系）这副"骨架"摆到理想位置，再点一次切换回「显示属性」——属性会自动根据当前矩形的位置围绕其均匀分布，避免一上来就被密集的属性椭圆干扰拖动。
